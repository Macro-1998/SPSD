...
class EnhancedMMT(BaseModel):
  ...
  def __init__(self,
    ...
    txt_middle_layers = self.txt_layers[1:-1]
    vid_middle_layers = self.vid_layers[1:-1]
    self.need_middle = (txt_middle_layers and vid_middle_layers)
    if self.need_middle:
      print('Debug ---> middle layers are used to compute contrastive loss after \'{}\' fusion.'.format(hi_fusion))
      self.hi_fusion = hi_fusion
      if self.hi_fusion == 'concat':
        err_msg = 'when \'concat\' fusion is used, middle layers of text encoder must have the same number with that of video encoder'
        assert len(txt_middle_layers) == len(vid_middle_layers), err_msg
        reduction_scale = 2
        self.middle_rep_dim = self.rep_dim // reduction_scale
      elif self.hi_fusion == 'mean':
        self.middle_rep_dim = self.rep_dim
      # transform tokens to representations
      self.txt_token2rep_middle = nn.ModuleList(ReduceDim(text_dim, self.middle_rep_dim) for _ in range(len(txt_middle_layers)))
      self.vid_token2rep_middle = nn.ModuleList(ReduceDim(same_dim, self.middle_rep_dim) for _ in range(len(vid_middle_layers)))
      # project text and video representations into common space
      if projector_layer == 'linear':
        if self.hi_fusion == 'concat':
          self.projector_middle = Projector([self.middle_rep_dim * len(txt_middle_layers), self.proj_dim])
        elif self.hi_fusion == 'mean':
          self.projector_middle = Projector([self.middle_rep_dim, self.proj_dim])
      elif projector_layer == 'nonlinear':
        if self.hi_fusion == 'concat':
          self.projector_middle = Projector([self.middle_rep_dim * len(txt_middle_layers), self.rep_dim, self.proj_dim])
        elif self.hi_fusion == 'mean':
          self.projector_middle = Projector([self.middle_rep_dim, self.rep_dim, self.proj_dim])
      # generate each token's weight for subsequent selecting representative ones
      if self.hi_fusion == 'concat':
        self.txt_w_gen_middle = GenWeight(self.middle_rep_dim * len(txt_middle_layers), 4)
        self.vid_w_gen_middle = GenWeight(self.middle_rep_dim * len(vid_middle_layers), 4)
      elif self.hi_fusion == 'mean':
        self.txt_w_gen_middle = GenWeight(self.middle_rep_dim, 4)
        self.vid_w_gen_middle = GenWeight(self.middle_rep_dim, 4)
  ...
  def forward(self,
    ...
    if self.txt_agg[:4] in ['bert']:
      ...
      # middle layers
      middle_layers = self.txt_layers[1:-1]
      if middle_layers:
        text_reps_middle = [all_layers[layer_id] for layer_id in middle_layers]
        for i in range(len(text_reps_middle)):
          text_reps_middle[i] = (self.txt_token2rep_middle[i])(text_reps_middle[i])
        if self.hi_fusion == 'concat':
          text_reps_middle = th.cat(text_reps_middle, dim=-1)
        elif self.hi_fusion == 'mean':
          text_reps_middle = th.stack(text_reps_middle, dim=0)  # [n, 128, 30, 768]
          text_reps_middle = th.mean(text_reps_middle, dim=0)
        text_cls_rep_middle = text_reps_middle[:, 0]
        text_token_rep_middle = text_reps_middle[:, 1:]
      # feature layers
    ...
    if self.vid_cont == 'bert':
      ...
      # middle layers
      middle_layers = self.vid_layers[1:-1]
      if middle_layers:
        vid_reps_middle = [all_layers[layer_id] for layer_id in middle_layers]
        for i in range(len(vid_reps_middle)):
          vid_reps_middle[i] = (self.vid_token2rep_middle[i])(vid_reps_middle[i])
        if self.hi_fusion == 'concat':
          vid_reps_middle = th.cat(vid_reps_middle, dim=-1)
        elif self.hi_fusion == 'mean':
          vid_reps_middle = th.stack(vid_reps_middle, dim=0)  # [n, 128, 30, 768]
          vid_reps_middle = th.mean(vid_reps_middle, dim=0)
        vid_cls_rep_middle = vid_reps_middle[:, 0]
        vid_token_rep_middle = vid_reps_middle[:, 1:]
      # feature layer
    ...
    # --------------------------------------------------------------------------------- middle layers projection -------------
    n_middle_layers_txt = len(self.txt_layers[1:-1])
    n_middle_layers_vid = len(self.vid_layers[1:-1])
    isNeedMiddle = n_middle_layers_vid > 0 and n_middle_layers_txt > 0
    if isNeedMiddle:
      # select representative text tokens
      if self.txt_ratio == 1:
        text_selected_token_rep_middle = text_token_rep_middle
      else:
        text_selected_token_rep_middle = self.select_representative_tokens(text_token_rep_middle, 'txt', 'middle')
      # select representive video tokens
      if self.vid_ratio == 1:
        vid_selected_token_rep_middle = vid_token_rep_middle
      else:
        vid_selected_token_rep_middle = self.select_representative_tokens(vid_token_rep_middle, 'vid', 'middle')
      # do average pooling on all tokens
      text_avg_pool_rep_middle = th.mean(text_token_rep_middle, dim=1)
      vid_avg_pool_rep_middle = th.mean(vid_token_rep_middle, dim=1)
      # project avg-pooled tokens ([b, rep_dim]) into common space ([b, proj_dim])
      if self.embd_out == 'avg':
        text_embd_middle = self.projector_middle(text_avg_pool_rep_middle)
        vid_embd_middle = self.projector_middle(vid_avg_pool_rep_middle)
      else:
        text_embd_middle = self.projector_middle(text_cls_rep_middle)
        vid_embd_middle = self.projector_middle(vid_cls_rep_middle)
      # coarse-grained-interaction: compute cross view confusion matrix for contrastive loss
      cross_view_conf_matrix_middle = sharded_cross_view_inner_product(vid_rep=vid_embd_middle, txt_rep=text_embd_middle)
      # project selected representative tokens ([b, n_s_token, rep_dim]) into common space ([b, n_s_token, proj_dim])
      vid_selected_embds_middle = self.projector_middle(vid_selected_token_rep_middle)
      text_selected_embds_rep_middle = self.projector_middle(text_selected_token_rep_middle)
      # fine-grained-interaction: compute mean-maximum similarity for self-distillation loss
      late_interaction_matrix_middle = cpt_mean_maximum_similarity(vid_selected_embds_middle, text_selected_embds_rep_middle)
    ...
    if isNeedMiddle:
      ...
      'cross_view_conf_matrix_middle': cross_view_conf_matrix_middle,
      ...
      'late_interaction_matrix_middle': late_interaction_matrix_middle,
