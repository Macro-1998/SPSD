...
class ActivityNet(BaseDataset):
  ...
  def configure_train_test_splits(self, cut_name, split_name):
    if cut_name in ["val1"]:
      ...
      if split_name in ["train", "trn", "val", "trainval"]:
        ...
        cross_vid_list = self.check(cross_vid_list)
  ...
  def check(self, cross_vid_list):
    new_cross_list = []
    dir_path = '/data/datasets/video-to-text/activity-net/vid_feat_files/mult_h5'
    #valid_vids = set([os.path.basename(x).rstrip('.h5') for x in glob.glob('/data/datasets/video-to-text/activity-net/vid_feat_files/mult_h5/v/_/*/*.h5')])
    n_error = 0
    for x in cross_vid_list:
      if os.path.exists(os.path.join(dir_path, "{}/{}/{}/{}.h5".format(x[0],x[1],x[2],x))):
        new_cross_list.append(x)
      else:
        n_error += 1
